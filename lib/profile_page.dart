...
class _ProfilePageState extends State<ProfilePage>
  ...
  // ── Always read live from Hive — never stale after class switch ─────────
  ...
  final List<Map<String, dynamic>> _completedCourses = [
    {
      'subject': 'Mathematics', 'subtitle': 'Quadratic Equations',
      'emoji': '🔢', 'c1': _C.skyA, 'c2': _C.skyB, 'progress': 1.0,
    },
    {
      'subject': 'Science', 'subtitle': 'Photosynthesis',
      'emoji': '🧪', 'c1': _C.mintA, 'c2': _C.mintB, 'progress': 1.0,
    },
  ];
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildStreakCard(),       // ← NEW real streak card
                              const SizedBox(height: 22),
  ...
  // ── Hero header — CHANGED: real username + board/class ───────────────────
              ...
              child: Column(children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    const _HeaderBadge(emoji: '🏆', label: '22,560 pts'),
                    const SizedBox(width: 8),
                    const _HeaderBadge(emoji: '📚', label: '42 Lessons'),
                  ],
                ),
              ]),
            ),
  ...
  // ── NEW: Real streak card with day-by-day dots ───────────────────────────
  Widget _buildStreakCard() {
    // Show last 7 days as dots — filled = watched that day
    // We can only check today and streak count; for past days we infer from streak
        ...
        Row(
          ...
          children: List.generate(7, (i) {
            ...
            final dayLabel  = ['M','T','W','T','F','S','S']
                [dayDate.weekday - 1]; // weekday 1=Mon

            // A day is "filled" if it falls within the current streak window
            // today (offset=0) filled if watchedToday
            // previous days filled based on streak count
            bool filled;
            if (dayOffset == 0) {
              filled = _watchedToday;
            } else {
              // within streak? streak covers today...(streak-1) days back
              // if not watched today, streak covers yesterday...(streak-1) back
  ...
  Widget _buildCoursesRow() {
    return Row(
      children: _completedCourses.asMap().entries.map((e) {
        final i = e.key; final course = e.value;
        return Expanded(
          child: Padding(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              height: 90,
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: [course['c1'], course['c2']],
                  begin: Alignment.topLeft, end: Alignment.bottomRight),
                borderRadius: const BorderRadius.only(
                  topLeft: Radius.circular(20), topRight: Radius.circular(20))),
              child: Center(child: Text(course['emoji'],
                  style: const TextStyle(fontSize: 36)))),
            Padding(
              padding: const EdgeInsets.all(12),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(course['subject'], style: const TextStyle(fontSize: 13,
                  fontWeight: FontWeight.w900, color: _C.inkDark)),
                const SizedBox(height: 2),
                Text(course['subtitle'], style: const TextStyle(fontSize: 10,
                ...
                ClipRRect(borderRadius: BorderRadius.circular(6),
                  child: LinearProgressIndicator(value: course['progress'],
                    ...
                    valueColor: AlwaysStoppedAnimation<Color>(course['c2']))),
